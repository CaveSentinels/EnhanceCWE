...
def post_save_send_email(sender, instance, created, using, **kwargs):
    ...
    random_key = instance.key
    email_add = instance.email
    subject = 'Enhanced CWE Invite'
    site_path = reverse("account_signup")
    machine_add = current_site_url()
    send_email(instance, subject, [email_add], random_key, site_path, machine_add)

def send_email(instance, subject, email, key, site_path, machine_add):
    """
    This method is used to send email
    :param: subject -the subject line of the email
    :param: email address list - the list of email addresses
    :param: key - the random generated authentication key
    :param: site_path - the application path
    :param: machine_add - the machine IP
    """
    if email:
